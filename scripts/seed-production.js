 ...
 *
 * It deliberately does NOT create any of the fabricated material that
 * scripts/seed.js generates for local use: no invented student accounts, no
 * synthetic reviews, no made-up bookings, no page-view traffic, and no
 * stand-in "pending approval" listings or payment receipts.
 *
 ...
 * claims a listing later. Attributing them to an invented account now would
 * be exactly the false data this script exists to avoid.
 *
 * Usage:
 *   ADMIN_EMAIL=… ADMIN_PASSWORD=… node scripts/seed-production.js
 *   node scripts/seed-production.js --purge-demo   # also strip generated data
 */
...
/** Records written by scripts/seed.js, listed here so they can be removed. */
...
async function purgeDemoData() {
  ...
  // Reviews, bookings and page views are all generated by scripts/seed.js,
  // so these collections are emptied wholesale rather than filtered.
  const results = {
  ...
  console.log('purged generated data:');
