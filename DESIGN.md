...
Type runs **Airbnb Cereal VF** (a custom variable font Airbnb licenses), with **Circular** as the historic in-house fallback and a system stack underneath. Cereal sits at modest weights, so display headlines render at 22–28px in weight 500–600, not the heavy 700+ weights that financial or enterprise systems lean on. The hero h1 ("Inspiration for future getaways") on the homepage is just 28px / 700, which would feel small on a typical SaaS page; here it works because the layout leans on photography (city collage, property cards) for visual weight rather than typographic muscle.

The shape language is **soft**. Buttons are 8px radius (`{rounded.sm}`), property cards are ~14px (`{rounded.md}`), the search bar is fully pill-shaped (`{rounded.full}`), wishlist hearts and search orbs are circles (`{rounded.full}`), and category strip rounded corners run at 32px (`{rounded.xl}`). There is essentially no hard corner anywhere except the body grid itself, because every interactive element is rounded.

**Key Characteristics:**
- Single accent color: `{colors.primary}` (#ff385c, "Rausch") carries every primary CTA, the search orb, the heart save state, and the brand wordmark. It is used scarcely; most pages are 90% white + ink with one or two Rausch moments.
...
- Three-product top nav: Homes, Experiences and Services, each with a hand-illustrated 32px icon and "NEW" badges (`{component.new-tag}`) on the two newer products. Active tab uses an underline rule (`{component.product-tab-active}`).
...
- Editorial dropdowns (footer, language picker) are clean text columns over the white canvas, with no card surface and no shadow.
...
- 8px base spacing system, with major sections at `{spacing.section}` (64px). That is generous but not airy enough to feel editorial-magazine; the marketplace density wants more cards per scroll.
...
- **Surface Strong** (`{colors.surface-strong}`, #f2f2f2): Slightly heavier fill for the circular icon-button surface (e.g., the breadcrumb back-arrow and listing toolbar buttons).

### Hairlines & Borders
- **Hairline** (`{colors.hairline}`, #dddddd): The default 1px border tone, used on search bar dividers, table separators, footer column splitters, and card 1px borders.
...
### Semantic
- **Error** (`{colors.primary-error-text}`, #c13515): Inline error text for form validation. Distinct from Rausch, being slightly darker and more saturated.
...
### Scrim
- **Scrim** (`{colors.scrim}`, #000000 at 50% opacity): The global modal backdrop tone for the date picker, login dialog and language picker. Stored as the base hex; opacity is applied at render time.
...
The single typographically loud moment in the entire system is the **rating display** (`{typography.rating-display}`, 64px / 700) on listing pages. That is the only place the system trusts type alone to carry hierarchy, because rating numbers are a peak trust signal and so get the loudest treatment.
...
The system gives editorial bands 64px of vertical breathing room but compresses card grids, with property and city-link cards sitting just 16px apart. The contrast is intentional: the page reads as "open hero, dense marketplace below," reinforcing the marketplace nature without overwhelming the visitor at the fold.

## Elevation

The system has essentially **one shadow tier** plus the flat baseline.

- **Flat (no shadow):** Body, hero, footer, all editorial bands, which is 95% of surfaces.
...
- **Modal scrim:** `{colors.scrim}` rendered at 50% opacity as the global modal backdrop. Used on date pickers, login dialogs, language picker.
...
**`property-card`**: A photo-first card. 1:1 aspect-ratio image with `{rounded.md}` corner clipping, image carousel dots overlay, "Guest favorite" floating badge top-left (`{component.guest-favorite-badge}`), and a heart icon top-right (`{component.icon-button-circle}` in default outlined state, Rausch-filled when saved). Beneath the image sit 4–5 lines of meta: title (`{typography.title-md}`), distance / dates (`{typography.body-sm}` muted), and price ("$X night") right-aligned.
...
**`text-input`**: White surface, 1px hairline outline, `{rounded.sm}` 8px radius, 56px height, 14×12px padding. Stacked label above (in `{typography.caption}` muted), placeholder text in `{typography.body-md}` muted. On focus, the border thickens to 2px ink and the border color flips to `{colors.ink}`, with no glow and no ring.

### Footer

**`footer-light`**: White surface (it matches the page canvas, because Airbnb has no contrast footer), 48×80px padding. Three columns of link blocks (Support / Hosting / Airbnb), separated by generous 24px gutters. Each column heads with a `{typography.title-sm}` ink label and stacks `{component.footer-link}` rows in `{typography.body-sm}` ink.
...
- Property and city-link grids drop column counts cleanly at each breakpoint. They never reflow rows; they always reduce columns.
...
- **Hover state colors:** intentionally not documented, per the global no-hover policy. Airbnb's actual `:hover` styling for property cards is a subtle elevation lift.
- **Loading states / skeleton screens:** not documented.
- **Map view styling:** the search-results map uses Mapbox-tinted tiles with custom Rausch markers, which are not documented here.
- **Form input error states:** the error text color (`{colors.primary-error-text}`) is documented, but the full input outline plus helper-text combination on validation failure is not.
- **Sub-brand palettes:** Luxe (`{colors.luxe}`) and Plus (`{colors.plus}`) are documented as tokens, but their full sub-system (typography overrides, surface treatment) lives on separate sub-domains and is not documented here.
