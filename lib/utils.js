...
/** PKR with no decimals, since rents are always whole rupees. */
...
/**
 * Photos uploaded after the last deploy are served by NGINX from
 * public/uploads, but `next start` only knows the public files that existed
 * when it booted, so the image optimiser answers 400 for them. Those photos are
 * sent as they are instead of going through the optimiser.
 */
export function isRuntimeUpload(src) {
