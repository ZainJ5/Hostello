...
/**
 * Returns a configured transport, or null when SMTP_HOST is unset. A null
 * transport is the intended development path: codes are logged to the server
 * console so signup works without a mail server.
 */
...
/**
 * One delivery path for every message. Resend wins when its key is present,
 * SMTP is the fallback for self-hosting, and with neither configured the
 * message is logged so local development still works end to end.
 *
 * Returns `{ delivered }` — false means nothing left the process.
 */
...
function layout({ title, intro, code, footer }) {
  // Table-based markup with inline styles — the only thing Outlook and Gmail
...
/**
 * Sends a verification code. With no SMTP configured the code is printed to
 * the server console and the call still resolves, so local signup works.
 * Returns { delivered } so callers can surface the dev path in the UI.
 */
