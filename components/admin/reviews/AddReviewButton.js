...
function toForm(review) {
  ...
  return {
    hostelId: String(review.hostelId || review.hostel?._id || ''),
    studentName: review.studentName || '',
    rating: review.rating || 0,
    cleanliness: review.cleanliness || 0,
    food: review.food || 0,
    security: review.security || 0,
...
/**
 * The add and edit form. Pass `review` to edit an existing one, leave it out
 * to add a new one.
 */
export function ReviewFormModal({ open, onClose, listings, review }) {
  ...
  // Reload the form whenever the dialog opens for a different review.
  const key = open ? review?._id || 'new' : null;
  if (key !== openedFor) {
    setOpenedFor(key);
    if (open) {
  ...
  async function save() {
    ...
    if (!editing) delete body.ownerReply;
    const res = editing
      ? await apiSend(`/api/admin/reviews/${review._id}`, {
          method: 'PATCH',
          body: { action: 'edit', review: body },
        })
      : await apiSend('/api/admin/reviews', { method: 'POST', body });
    setSaving(false);
    if (!res.ok) {
      setErrors(
      ...
      return toast({
        tone: 'danger',
        title: editing ? 'Could not save review' : 'Could not add review',
        description: res.error,
      });
    }
    const name = listings.find((l) => l.value === form.hostelId)?.label || 'The listing';
    if (res.data?.unchanged) {
      toast({ tone: 'info', title: 'Nothing changed', description: 'The review is the same as before.' });
    } else {
      toast({
        title: editing ? 'Review updated' : 'Review added',
  ...
  return (
    <Modal
      ...
      description={
        editing
          ? 'Changes show on the listing straight away and the rating is recalculated.'
          : 'For feedback a student gave you on WhatsApp, a call or a visit. It counts toward the listing rating like any other review.'
      }
      footer={
        <>
      ...
      <div className="space-y-4">
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <Input
            label="Review date"
            type="date"
            hint={editing ? 'Change it only to correct a wrong date' : 'Leave empty to use today'}
        ...
        {editing && (
          <Textarea
            ...
            onChange={(e) => set('ownerReply', e.target.value)}
            error={errors.ownerReply}
            hint="Clear it to remove the reply"
            maxLength={2000}
          />
        )}
...
export default function AddReviewButton({ listings }) {
  ...
  return (
    <>
      ...
      <ReviewFormModal open={open} onClose={() => setOpen(false)} listings={listings} />
    </>
  );
}
